...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not new_place.is_hive:
            if new_place.bees and self.lower_bound <= k <= self.upper_bound:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew == 0:
            if self.place.bees:
                beee = random_bee(self.place.bees)
                while beee.health > 0:
                    beee.reduce_health(1)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        if cls.food_cost > gamestate.food:
            return None
        if gamestate.queenon == True:
            gamestate.queenon = False
    ...
    def action(self, gamestate):
        ...
        while check_place:
            if check_place.ant:
                if check_place.ant.is_doubled:
                    check_place.ant.double()
                    check_place.ant.is_doubled = False
                if check_place.ant.is_container and check_place.ant.ant_contained:
                    if check_place.ant.ant_contained.is_doubled == True:

                        check_place.ant.ant_contained.double()
                        check_place.ant.ant_contained.is_doubled = False
                check_place.ant.is_doubled = False
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target:
            ...
            target.action = lambda x: action(x)
