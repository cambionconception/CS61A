...
def perfect_square(x, k=0, checker=False):
    while x>k:
        k=k+1
        if x/k == k:
            checker=True    
...
def make_averaged(original_function, total_samples=1000):
    ...
    def bigbooling(*args):
        ...
        while a > 0:
            a = a-1
            b = b + original_function(*args)
        return b/total_samples    
    return bigbooling
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while x<=10:
        currentroll = averaged_dice(x,dice)
        if currentroll > a:
            a = currentroll
            correctanswer = x
        x = x + 1
