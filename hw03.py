...
def pingpong(n):
    ...
    True
    """
    return alastair(n,1,1,1)


def alastair(n,counter,current,upordown):
    if counter == n:
        return current
    else:
        if num_eights(counter) > 0 or counter % 8 == 0:
            return alastair(n,counter+1,current-upordown,-upordown)
        else:
            return alastair(n, counter+1,current+upordown, upordown)
...
def count_coins(change):
    ...
    True
    """
    def helper_function(change, current_coin):
        if change == 0:
            return 1
        elif change < 0 or current_coin == None:
            return 0
        return helper_function(change - current_coin, current_coin) + helper_function(change, next_larger_coin(current_coin))
    return helper_function(change, 1)
