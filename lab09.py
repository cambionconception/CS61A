...
def partition_gen(n):
    ...
    def yield_helper(j, k):
        if j == 0:
            ...
            for small_part in yield_helper(j-k):
...
class VendingMachine:
    ...
    def add_funds(self,y):
        if self.stock == 0:
            return 'Nothing left to vend. Please restock. Here is your $' + str(y) + '.'
        else:
            self.funds = self.funds + y
    ...
    def vend(self):
        ...
        if self.stock == 0:
            ...
            return 'Please update your balance with $' + str(self.price - self.funds) + ' more funds.'
        else:
